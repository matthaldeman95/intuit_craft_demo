...
referred to as the data buffer***REMOVED***, and the information about the raw array data.
The data buffer is typically what people think of as arrays in C or Fortran,
a contiguous (and fixed***REMOVED*** block of memory containing fixed sized data items.
Numpy also contains a significant set of data that describes how to interpret
the data in the data buffer. This extra information contains (among other things***REMOVED***:

 1***REMOVED*** The basic data element's size in bytes
 2***REMOVED*** The start of the data within the data buffer (an offset relative to the
    beginning of the data buffer***REMOVED***.
 3***REMOVED*** The number of dimensions and the size of each dimension
 4***REMOVED*** The separation between elements for each dimension (the 'stride'***REMOVED***. This
    does not have to be a multiple of the element size
 5***REMOVED*** The byte order of the data (which may not be the native byte order***REMOVED***
 6***REMOVED*** Whether the buffer is read-only
 7***REMOVED*** Information (via the dtype object***REMOVED*** about the interpretation of the basic
    data element. The basic data element may be as simple as a int or a float,
    or it may be a compound object (e.g., struct-like***REMOVED***, a fixed character field,
    or Python object pointers.
 8***REMOVED*** Whether the array is to interpreted as C-order or Fortran-order.
...
of the buffer (e.g., different shape, offset, byte order, strides, etc***REMOVED*** but
...
use of the .copy(***REMOVED*** method if one really wants to make a new and independent
...
first index represents x position (i.e., column***REMOVED*** and the second represents y
position (i.e., row***REMOVED***. This alone is the source of much confusion;
...
time (since the first index moves to the next row as it changes***REMOVED***. Thus Fortran
...
matrices***REMOVED*** stored in data files. Suppose the data are stored by rows rather than
by columns. If we are to preserve our index convention (whether matrix or
image***REMOVED*** that means that depending on the language we use, we may be forced to
...
order options for array ordering in numpy.***REMOVED*** The drawback of doing this is
...
that im[0, 10***REMOVED*** represents the value at x=0, y=10. To be consistent with usual
Python behavior then im[0***REMOVED*** would represent a column at x=0. Yet that data
...
contiguous subarrays is still awkward (e.g., im[:,0***REMOVED*** for the first row, vs
im[0***REMOVED******REMOVED***, thus one can't use an idiom such as for row in im; for col in im does
...
actually***REMOVED*** are not contiguous in memory.
...
and the last gets the smallest element***REMOVED***. Since image data are normally stored
...
there are two approaches to consider: 1***REMOVED*** accept that the first index is just not

...
#. Explicit constructor call - as in ``MySubClass(params***REMOVED***``.  This is
...
(specified***REMOVED*** subclass:

>>> import numpy as np
>>> # create a completely useless ndarray subclass
>>> class C(np.ndarray***REMOVED***: pass
>>> # create a standard ndarray
>>> arr = np.zeros((3,***REMOVED******REMOVED***
>>> # take a view of it, as our useless subclass
>>> c_arr = arr.view(C***REMOVED***
>>> type(c_arr***REMOVED***
<class 'C'>
...
>>> v = c_arr[1:***REMOVED***
>>> type(v***REMOVED*** # the view is of type 'C'
<class 'C'>
...
such as copying arrays (``c_arr.copy(***REMOVED***``***REMOVED***, creating ufunc output arrays
(see also :ref:`array-wrap`***REMOVED***, and reducing methods (like
``c_arr.mean(***REMOVED***``.
...
.. testcode::
  ...
  class C(object***REMOVED***:
      def __new__(cls, *args***REMOVED***:
          print('Cls in __new__:', cls***REMOVED***
          print('Args in __new__:', args***REMOVED***
          return object.__new__(cls, *args***REMOVED***

      def __init__(self, *args***REMOVED***:
          print('type(self***REMOVED*** in __init__:', type(self***REMOVED******REMOVED***
          print('Args in __init__:', args***REMOVED***

meaning that we get:

>>> c = C('hello'***REMOVED***
Cls in __new__: <class 'C'>
Args in __new__: ('hello',***REMOVED***
type(self***REMOVED*** in __init__: <class 'C'>
Args in __init__: ('hello',***REMOVED***

When we call ``C('hello'***REMOVED***``, the ``__new__`` method gets its own class
as first argument, and the passed argument, which is the string
``'hello'``.  After python calls ``__new__``, it usually (see below***REMOVED***
calls our ``__init__`` method, with the output of ``__new__`` as the
first argument (now a class instance***REMOVED***, and the passed arguments
following.
...
.. testcode::
  ...
  class D(C***REMOVED***:
      def __new__(cls, *args***REMOVED***:
          print('D cls is:', cls***REMOVED***
          print('D args in __new__:', args***REMOVED***
          return C.__new__(C, *args***REMOVED***

      def __init__(self, *args***REMOVED***:
          # we never get here
          print('In D __init__'***REMOVED***

meaning that:

>>> obj = D('hello'***REMOVED***
D cls is: <class 'D'>
D args in __new__: ('hello',***REMOVED***
Cls in __new__: <class 'C'>
Args in __new__: ('hello',***REMOVED***
>>> type(obj***REMOVED***
<class 'C'>
...
have a ``__new__`` method with the same call signature***REMOVED***.
...
#. explicit constructor call (``obj = MySubClass(params***REMOVED***``***REMOVED***.  This will
   call the usual sequence of ``MySubClass.__new__`` then (if it exists***REMOVED***
   ``MySubClass.__init__``.
#. :ref:`view-casting`
...
* For the explicit constructor call, our subclass will need to create a
  ...
  ``ndarray.__new__(MySubClass,...***REMOVED***``, or do view casting of an existing
  array (see below***REMOVED***
...
.. testcode::
   ...
   class C(np.ndarray***REMOVED***:
       def __new__(cls, *args, **kwargs***REMOVED***:
           print('In __new__ with class %s' % cls***REMOVED***
           return np.ndarray.__new__(cls, *args, **kwargs***REMOVED***

       def __init__(self, *args, **kwargs***REMOVED***:
           # in practice you probably will not need or want an __init__
           # method for your subclass
           print('In __init__ with class %s' % self.__class__***REMOVED***

       def __array_finalize__(self, obj***REMOVED***:
           print('In array_finalize:'***REMOVED***
           print('   self type is %s' % type(self***REMOVED******REMOVED***
           print('   obj type is %s' % type(obj***REMOVED******REMOVED***


Now:

>>> # Explicit constructor
>>> c = C((10,***REMOVED******REMOVED***
...
>>> cast_a = a.view(C***REMOVED***
In array_finalize:
   self type is <class 'C'>
   obj type is <type 'numpy.ndarray'>
>>> # Slicing (example of new-from-template***REMOVED***
>>> cv = c[:1***REMOVED***
In array_finalize:
   self type is <class 'C'>
   obj type is <class 'C'>
...
own class (``self``***REMOVED*** as well as the object from which the view has been
taken (``obj``***REMOVED***.  As you can see from the output above, the ``self`` is
...
.. testcode::
  ...
  class InfoArray(np.ndarray***REMOVED***:

      def __new__(subtype, shape, dtype=float, buffer=None, offset=0,
            strides=None, order=None, info=None***REMOVED***:
      ...
      def __array_finalize__(self, obj***REMOVED***:
          ...
          # ndarray.__new__(InfoArray, ...***REMOVED***, therefore it only has
          # attributes that the ndarray.__new__ constructor gave it -
          # i.e. those of a standard ndarray.
          #
          # We could have got to the ndarray.__new__ call in 3 ways:
          # From an explicit constructor - e.g. InfoArray(***REMOVED***:
          #    obj is None
          #    (we're in the middle of the InfoArray.__new__
          #    constructor, and self.info will be set when we return to
          #    InfoArray.__new__***REMOVED***
          if obj is None: return
          # From view casting - e.g arr.view(InfoArray***REMOVED***:
          #    obj is arr
          #    (type(obj***REMOVED*** can be InfoArray***REMOVED***
          # From new-from-template - e.g infoarr[:3***REMOVED***
          #    type(obj***REMOVED*** is InfoArray
          #
          # Note that it is here, rather than in the __new__ method,
          # that we set the default value for 'info', because this
          # method sees all creation of default objects - with the
          # InfoArray.__new__ constructor, but also with
          # arr.view(InfoArray***REMOVED***.
          self.info = getattr(obj, 'info', None***REMOVED***
          # We do not need to return anything


Using the object looks like this:

  >>> obj = InfoArray(shape=(3,***REMOVED******REMOVED*** # explicit constructor
  >>> type(obj***REMOVED***
  <class 'InfoArray'>
  >>> obj.info is None
  True
  >>> obj = InfoArray(shape=(3,***REMOVED***, info='information'***REMOVED***
  >>> obj.info
  'information'
  >>> v = obj[1:***REMOVED*** # new-from-template - here - slicing
  >>> type(v***REMOVED***
  <class 'InfoArray'>
  >>> v.info
  'information'
  >>> arr = np.arange(10***REMOVED***
  >>> cast_arr = arr.view(InfoArray***REMOVED*** # view casting
  >>> type(cast_arr***REMOVED***
  <class 'InfoArray'>
...
.. testcode::
  ...
  class RealisticInfoArray(np.ndarray***REMOVED***:

      def __new__(cls, input_array, info=None***REMOVED***:
          # Input array is an already formed ndarray instance
          # We first cast to be our class type
          obj = np.asarray(input_array***REMOVED***.view(cls***REMOVED***
...
So:
  ...
  >>> arr = np.arange(5***REMOVED***
  >>> obj = RealisticInfoArray(arr, info='information'***REMOVED***
  >>> type(obj***REMOVED***
...
.. testcode::
  ...
  class MySubClass(np.ndarray***REMOVED***:
      ...
      def __array_finalize__(self, obj***REMOVED***:
          print('In __array_finalize__:'***REMOVED***
          print('   self is %s' % repr(self***REMOVED******REMOVED***
          print('   obj is %s' % repr(obj***REMOVED******REMOVED***
          if obj is None: return
          self.info = getattr(obj, 'info', None***REMOVED***

      def __array_wrap__(self, out_arr, context=None***REMOVED***:
          print('In __array_wrap__:'***REMOVED***
          print('   self is %s' % repr(self***REMOVED******REMOVED***
          print('   arr is %s' % repr(out_arr***REMOVED******REMOVED***
          # then just call the parent
          return np.ndarray.__array_wrap__(self, out_arr, context***REMOVED***

We run a ufunc on an instance of our new array:

>>> obj = MySubClass(np.arange(5***REMOVED***, info='spam'***REMOVED***
In __array_finalize__:
   self is MySubClass([0, 1, 2, 3, 4***REMOVED******REMOVED***
   obj is array([0, 1, 2, 3, 4***REMOVED******REMOVED***
>>> arr2 = np.arange(5***REMOVED***+1
>>> ret = np.add(arr2, obj***REMOVED***
In __array_wrap__:
   self is MySubClass([0, 1, 2, 3, 4***REMOVED******REMOVED***
   arr is array([1, 3, 5, 7, 9***REMOVED******REMOVED***
In __array_finalize__:
   self is MySubClass([1, 3, 5, 7, 9***REMOVED******REMOVED***
   obj is MySubClass([0, 1, 2, 3, 4***REMOVED******REMOVED***
>>> ret
MySubClass([1, 3, 5, 7, 9***REMOVED******REMOVED***
>>> ret.info
'spam'

Note that the ufunc (``np.add``***REMOVED*** has called the ``__array_wrap__`` method of the
input with the highest ``__array_priority__`` value, in this case
``MySubClass.__array_wrap__``, with arguments ``self`` as ``obj``, and
``out_arr`` as the (ndarray***REMOVED*** result of the addition.  In turn, the
default ``__array_wrap__`` (``ndarray.__array_wrap__``***REMOVED*** has cast the
...
.. testcode::
  ...
  class SillySubClass(np.ndarray***REMOVED***:

      def __array_wrap__(self, arr, context=None***REMOVED***:
          return 'I lost your data'

>>> arr1 = np.arange(5***REMOVED***
>>> obj = arr1.view(SillySubClass***REMOVED***
>>> arr2 = np.arange(5***REMOVED***
>>> ret = np.multiply(obj, arr2***REMOVED***
>>> ret
'I lost your data'
...
domain of the ufunc***REMOVED***. ``__array_wrap__`` should return an instance of
...
created an ndarray, ``arr`` and have taken a slice with ``v = arr[1:***REMOVED***``.
The two objects are looking at the same memory.  Numpy keeps track of
where the data came from for a particular array or view, with the
``base`` attribute:

>>> # A normal ndarray, that owns its own data
>>> arr = np.zeros((4,***REMOVED******REMOVED***

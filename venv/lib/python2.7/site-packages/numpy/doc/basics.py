...
==========  ==========================================================
bool_       Boolean (True or False***REMOVED*** stored as a byte
int_        Default integer type (same as C ``long``; normally either
            ``int64`` or ``int32``***REMOVED***
intc        Identical to C ``int`` (normally ``int32`` or ``int64``***REMOVED***
intp        Integer used for indexing (same as C ``ssize_t``; normally
            either ``int32`` or ``int64``***REMOVED***
int8        Byte (-128 to 127***REMOVED***
int16       Integer (-32768 to 32767***REMOVED***
int32       Integer (-2147483648 to 2147483647***REMOVED***
int64       Integer (-9223372036854775808 to 9223372036854775807***REMOVED***
uint8       Unsigned integer (0 to 255***REMOVED***
uint16      Unsigned integer (0 to 65535***REMOVED***
uint32      Unsigned integer (0 to 4294967295***REMOVED***
uint64      Unsigned integer (0 to 18446744073709551615***REMOVED***
...
complex64   Complex number, represented by two 32-bit floats (real
            and imaginary components***REMOVED***
...
Numpy numerical types are instances of ``dtype`` (data-type***REMOVED*** objects, each
...
There are 5 basic numerical types representing booleans (bool***REMOVED***, integers (int***REMOVED***,
unsigned integers (uint***REMOVED*** floating point (float***REMOVED*** and complex. Those with numbers
in their name indicate the bitsize of the type (i.e. how many bits are needed
to represent a single value in memory***REMOVED***.  Some types, such as ``int`` and
``intp``, have differing bitsizes, dependent on the platforms (e.g. 32-bit
vs. 64-bit machines***REMOVED***.  This should be taken into account when interfacing
with low-level code (such as C or Fortran***REMOVED*** where the raw memory is addressed.

Data-types can be used as functions to convert python numbers to array scalars
(see the array scalar section for an explanation***REMOVED***, python sequences of numbers
to arrays of that type, or as arguments to the dtype keyword that many numpy
functions or methods accept. Some examples::

    >>> import numpy as np
    >>> x = np.float32(1.0***REMOVED***
    >>> x
    1.0
    >>> y = np.int_([1,2,4***REMOVED******REMOVED***
    >>> y
    array([1, 2, 4***REMOVED******REMOVED***
    >>> z = np.arange(3, dtype=np.uint8***REMOVED***
    >>> z
    array([0, 1, 2***REMOVED***, dtype=uint8***REMOVED***
...
documentation may still refer to these, for example::
  ...
  >>> np.array([1, 2, 3***REMOVED***, dtype='f'***REMOVED***
  array([ 1.,  2.,  3.***REMOVED***, dtype=float32***REMOVED***

We recommend using dtype objects instead.

To convert the type of an array, use the .astype(***REMOVED*** method (preferred***REMOVED*** or
the type itself as a function. For example: ::

    >>> z.astype(float***REMOVED***                 #doctest: +NORMALIZE_WHITESPACE
    array([  0.,  1.,  2.***REMOVED******REMOVED***
    >>> np.int8(z***REMOVED***
    array([0, 1, 2***REMOVED***, dtype=int8***REMOVED***
...
properties of the type, such as whether it is an integer::
    ...
    >>> np.issubdtype(d, int***REMOVED***
    True

    >>> np.issubdtype(d, float***REMOVED***
    False
...
with an associated dtype***REMOVED***.  Array scalars differ from Python scalars, but
for the most part they can be used interchangeably (the primary
exception is for versions of Python older than v2.x, where integer array
scalars cannot act as indices for lists and tuples***REMOVED***.  There are some
exceptions, such as when code requires very specific attributes of a scalar
or when it checks specifically whether a value is a Python scalar. Generally,
problems are easily fixed by explicitly converting array scalars
to Python scalars, using the corresponding Python type function
(e.g., ``int``, ``float``, ``complex``, ``str``, ``unicode``***REMOVED***.

The primary advantage of using array scalars is that
they preserve the array type (Python may not have a matching scalar type
available, e.g. ``int16``***REMOVED***.  Therefore, the use of array scalars ensures
...
``long double`` type, MSVC (standard for Windows builds***REMOVED*** makes
``long double`` identical to ``double`` (64 bits***REMOVED***. Numpy makes the
compiler's ``long double`` available as ``np.longdouble`` (and
``np.clongdouble`` for the complex numbers***REMOVED***. You can find out what your
numpy provides with``np.finfo(np.longdouble***REMOVED***``.

Numpy does not provide a dtype with more precision than C
``long double``s; in particular, the 128-bit IEEE quad precision
data type (FORTRAN's ``REAL*16``***REMOVED*** is not available.
...
``1 + np.finfo(np.longdouble***REMOVED***.eps``.

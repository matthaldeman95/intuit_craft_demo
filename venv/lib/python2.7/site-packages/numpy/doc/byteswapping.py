...
bytes from a file written by a Sun (big-endian***REMOVED*** computer.  I know that
these 4 bytes represent two 16-bit integers.  On a big-endian machine, a
two-byte integer is stored with the Most Significant Byte (MSB***REMOVED*** first,
and then the Least Significant Byte (LSB***REMOVED***. Thus the bytes are, in memory order:
...
>>> big_end_str = chr(0***REMOVED*** + chr(1***REMOVED*** + chr(3***REMOVED*** + chr(2***REMOVED***
>>> big_end_str
'\\x00\\x01\\x03\\x02'
...
>>> import numpy as np
>>> big_end_arr = np.ndarray(shape=(2,***REMOVED***,dtype='>i2', buffer=big_end_str***REMOVED***
>>> big_end_arr[0***REMOVED***
1
>>> big_end_arr[1***REMOVED***
770

Note the array ``dtype`` above of ``>i2``.  The ``>`` means 'big-endian'
(``<`` is little-endian***REMOVED*** and ``i2`` means 'signed 2-byte integer'.  For
example, if our data represented a single unsigned 4-byte little-endian
integer, the dtype string would be ``<u4``.

In fact, why don't we try that?

>>> little_end_u4 = np.ndarray(shape=(1,***REMOVED***,dtype='<u4', buffer=big_end_str***REMOVED***
>>> little_end_u4[0***REMOVED*** == 1 * 256**1 + 3 * 256**2 + 2 * 256**3
True

Returning to our ``big_end_arr`` - in this case our underlying data is
big-endian (data endianness***REMOVED*** and we've set the dtype to match (the dtype
is also big-endian***REMOVED***.  However, sometimes you need to flip these around.
...
.. warning::
    ...
    >>> big_end_arr[0***REMOVED***.dtype.byteorder == little_end_u4[0***REMOVED***.dtype.byteorder
    True
...
* Change the byte-ordering information in the array dtype so that it
  ...
  This is the role of ``arr.newbyteorder(***REMOVED***``
* Change the byte-ordering of the underlying data, leaving the dtype
  interpretation as it was.  This is what ``arr.byteswap(***REMOVED***`` does.
...
>>> wrong_end_dtype_arr = np.ndarray(shape=(2,***REMOVED***,dtype='<i2', buffer=big_end_str***REMOVED***
>>> wrong_end_dtype_arr[0***REMOVED***
256

The obvious fix for this situation is to change the dtype so it gives
the correct endianness:

>>> fixed_end_dtype_arr = wrong_end_dtype_arr.newbyteorder(***REMOVED***
>>> fixed_end_dtype_arr[0***REMOVED***
1

Note the the array has not changed in memory:

>>> fixed_end_dtype_arr.tobytes(***REMOVED*** == big_end_str
True
...
>>> fixed_end_mem_arr = wrong_end_dtype_arr.byteswap(***REMOVED***
>>> fixed_end_mem_arr[0***REMOVED***
1

Now the array *has* changed in memory:

>>> fixed_end_mem_arr.tobytes(***REMOVED*** == big_end_str
False
...
>>> swapped_end_arr = big_end_arr.byteswap(***REMOVED***.newbyteorder(***REMOVED***
>>> swapped_end_arr[0***REMOVED***
1
>>> swapped_end_arr.tobytes(***REMOVED*** == big_end_str
False

An easier way of casting the data to a specific dtype and byte ordering
can be achieved with the ndarray astype method:

>>> swapped_end_arr = big_end_arr.astype('<i2'***REMOVED***
>>> swapped_end_arr[0***REMOVED***
1
>>> swapped_end_arr.tobytes(***REMOVED*** == big_end_str
False

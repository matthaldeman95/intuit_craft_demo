...
factors.
 ...
 1***REMOVED*** platform portability (i.e. Microsoft says not to use longjmp
     to return from signal handling.  They have a __try  and __except
     extension to C instead but what about mingw?***REMOVED***.

 2***REMOVED*** how to handle threads: apparently whether signals are delivered to
    every thread of the process or the "invoking" thread is platform
    dependent. --- we don't handle threads for now.

 3***REMOVED*** do we need to worry about re-entrance.  For now, assume the
    code will not call-back into itself.

Ideas:

 1***REMOVED*** Start by implementing an approach that works on platforms that
    can use setjmp and longjmp functionality and does nothing
    on other platforms.

 2***REMOVED*** Ignore threads --- i.e. do not mix interrupt handling and threads

 3***REMOVED*** Add a default signal_handler function to the C-API but have the rest
    use macros.
...
[code***REMOVED*** block must not allocate any memory or alter the reference count of any
Python objects.  In other words [code***REMOVED*** must be interruptible so that continuation
...
#define NPY_SIGSETJMP(arg1, arg2***REMOVED*** setjmp(arg1***REMOVED***
#define NPY_SIGLONGJMP(arg1, arg2***REMOVED*** longjmp(arg1, arg2***REMOVED***
#define NPY_SIGJMP_BUF jmp_buf

#else

#define NPY_SIGSETJMP(arg1, arg2***REMOVED*** sigsetjmp(arg1, arg2***REMOVED***
#define NPY_SIGLONGJMP(arg1, arg2***REMOVED*** siglongjmp(arg1, arg2***REMOVED***
...
#    define NPY_SIGINT_ON {                                             \
                   ...
                   _npy_sig_save = PyOS_setsig(SIGINT, _PyArray_SigintHandler***REMOVED***; \
                   if (NPY_SIGSETJMP(*((NPY_SIGJMP_BUF ****REMOVED***_PyArray_GetSigintBuf(***REMOVED******REMOVED***, \
                                 1***REMOVED*** == 0***REMOVED*** {                             \

#    define NPY_SIGINT_OFF ***REMOVED***                                      \
        PyOS_setsig(SIGINT, _npy_sig_save***REMOVED***;                       \
    ***REMOVED***
